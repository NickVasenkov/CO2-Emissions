...
def train_evaluate(params):
    ...
    # Multiply 'Median_train' by coeffs
    train.loc[train['year'] == 2021, 'Median_multiplied_train'] = \
        train.loc[train['year'] == 2021, 'Location_group'].map(params) * \
        train.loc[train['year'] == 2021, 'Median_train']
    print(len(train.loc[train['year'] == 2021, 'Median_multiplied_train'].unique()))

    score = mean_squared_error(emission, train.loc[train['year'] == 2021, 'Median_multiplied_train'], squared=False)
...
def objective(trial):
    params = {
        'Special 1': trial.suggest_float('Special 1', 0.8, 1.3, step=0.001),
        'Special 2': trial.suggest_float('Special 2', 0.8, 1.3, step=0.001),
        'High': trial.suggest_float('High', 0.8, 1.3, step=0.001),
        'Low': trial.suggest_float('Low', 0.8, 1.3, step=0.001),
    }
